  ...
  <div class="container-fluid" id="sel_proy_buscador">
      <div class="card-header">{{ __('Resultado Búsqueda programaciones Preliminares:'.$cant_resul.' Resultados Obtenidos') }}</div>
        <div class="row">
                ...
                <form action="{{route('export_list_proyecc.excel')}}" method="GET" name="proy_buscador">
                    ...
                    <table class="table table-bordered table-condensed table-hover table-sm header_table" cellspacing="0">
                        ...
                        @foreach ($programaciones as $item) 
                            ...
                            <td style="text-align:center;">
                                <input type="checkbox" id="programacion_list[]" name="programacion_list[]" value="{{ $item->id }}">
                            </td> 
                            <td>{{ $item->id }}</td>
                    ...
                    <div>
                        <label class="col-form-label text-md-right">
                            ...
                            data-title="El número máximo de programaciones releccionadas es de 300" style="font-size: 0.813rem"></i>
                             {{ __('programaciones Seleccionadas:') }}
                        </label>
                        <label id="cant_sel" name="cant_sel" class="col-form-label text-md-right">0
                        </label>
                    </div>
                    <a href="{{route('export_list_proyecc.excel')}}">
                        <button class="btn-success" style="background-color: #447161; border:0" name="export_programacion" id="export_programacion"    disabled><i class="fas fa-download" ></i>   XLS</button>
                    </a> 
                </form>
                <br>
            </div>
            ...
            {{-- {{$programaciones->appends(['sort' => "input[name='programacion_list[]']"])->links()}} --}}
        </div>
